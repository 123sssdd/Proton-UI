...
Proton UI 提供了丰富的赛博朋克风格视觉特效系统，包括霓虹光晕、玻璃拟态、粒子效果、像素化图片处理等。所有特效旨在打造沉浸式的现代科幻体验。
...
<DemoBox
  ...
  code={`import { PixelatedImage } from '@proton-ui/components';
...
<DocTabs>
  <DocTabPanel id="retro8bit" label="8-bit 风格">
    <DemoBox
      code={`import { PixelatedImage } from '@proton-ui/components';
import { PIXELATE_PRESETS } from '@proton-ui/utils';
...
// ❌ 避免过度叠加
<div className="glass-morphism neon-glow-cyan-box crt-full-effect">
  内容
</div>
```
