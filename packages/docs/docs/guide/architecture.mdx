...
Proton UI 采用 Monorepo 架构，基于 pnpm + Turborepo 构建，提供高效的开发体验和灵活的包管理。
...
Proton-ui/
├── packages/              # 核心包
│   ├── core/             # 元包 (Meta-package)
│   ├── components/       # UI 组件库 (Headless + Styled)
│   ├── hooks/            # React Hooks (Headless Logic)
│   ├── utils/            # 工具函数与类型
│   ├── streaming/        # 流式渲染引擎
│   └── docs/             # 文档站点
├── apps/                 # 应用
...
graph TB
    ...
    subgraph "组件层"
        C[@proton-ui/core]
        D[@proton-ui/components]
        E[@proton-ui/hooks]
        F[@proton-ui/utils]
        G[@proton-ui/streaming]
    end
...
元包 (Meta-package)，重新导出所有其它包的内容，确保向后兼容性。仅仅作为一个统一入口使用。

### @proton-ui/components

包含所有的 UI 组件（Headless 逻辑 + 默认样式）。

**主要功能：**

- 基础组件（Button, Input, Card）
- 对话组件（Message, ChatContainer）
- 视觉组件（ParticleEffect, PixelatedImage）

### @proton-ui/hooks

纯逻辑层，包含所有自定义 React Hooks，实现 Headless UI 理念。

**主要 Hooks：**

- `useParticles`: 粒子系统逻辑
- `usePixelTheme`: 主题系统逻辑
- `usePixelate`: 图片像素化逻辑

### @proton-ui/utils

底层工具库，包含共享的帮助函数和类型定义。
...
这些设计使得 Proton UI 既适合小型项目快速开发，也能支撑大型项目的长期维护。
